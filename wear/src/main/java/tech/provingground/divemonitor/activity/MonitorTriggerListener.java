...
import tech.provingground.divemonitor.DiveInitData;
import tech.provingground.divemonitor.model.GlobalContext;
import tech.provingground.divemonitor.services.LogTransferService;
...
public class MonitorTriggerListener extends WearableListenerService {
	...
	public void onMessageReceived(MessageEvent messageEvent) {
		...
		if (messageEvent.getPath().equals("/startMonitoring")) {
			...
			i.setAction("tech.provingground.divemonitor.START_MONITORING");
			...
			intent.setAction("tech.provingground.divemonitor.wear.TRANSFER_LOGS");
			startService(intent);
		} else if (messageEvent.getPath().equals("/logRetrievalComplete")) {
			Intent i = new Intent(this, LogTransferService.class);
			i.setAction("tech.provingground.divemonitor.wear.CLEAR_LOCAL_DB");
			startService(i);
		}
	}
}
